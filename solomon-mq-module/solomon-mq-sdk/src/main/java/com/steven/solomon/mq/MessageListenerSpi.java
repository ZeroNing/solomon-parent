 ...
 * 通用消息监听器 SPI。
 *
 * <p>RabbitMQ、RocketMQ、Kafka、MQTT、Redis 等消息组件共用这套消费模板：
 * 先把原始 JSON 转成消息模型，再把 body 转成真实业务泛型，最后交给具体消费者处理。</p>
 *
 * @param <T> 业务消息体类型
 * @param <R> 消费结果类型
 ...
 * @author steven
 */
public interface MessageListenerSpi<T, R, M extends BaseMq<T>> {
