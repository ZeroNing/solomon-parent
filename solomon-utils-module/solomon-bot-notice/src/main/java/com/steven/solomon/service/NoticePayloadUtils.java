...
final class NoticePayloadUtils {
    ...
    static String appendSignature(String content, boolean enabled, String signature, String prefix) {
        ...
        if (enabled && StrUtil.isNotBlank(signature)) { builder.append(StrUtil.nullToEmpty(prefix)).append(signature); }
    ...
    static String firstButtonUrlOrDefault(NoticeMessage message, String defaultUrl) { if (!hasItems(message.getButtons())) { return defaultUrl; } return message.getButtons().get(0).getActionUrl(); }

    static final class PayloadMap {
        private final Map<String, Object> data = new HashMap<>();
        PayloadMap put(String key, Object value) { data.put(key, value); return this; }
        PayloadMap putIf(boolean condition, String key, Object value) { if (condition) { data.put(key, value); } return this; }
        Map<String, Object> build() { return data; }
    }
}
