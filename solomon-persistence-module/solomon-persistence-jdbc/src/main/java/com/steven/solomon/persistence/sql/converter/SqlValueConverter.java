...
public interface SqlValueConverter {
   ...
   *
   * @param value 数据库返回的原始值，可能是Timestamp、BigDecimal、String�?
   ...
   * 把数据库值转换为指定Java类型�?
   *
   * @param value 数据库返回的原始�?
   * @param targetType 目标Java类型
   * @return 转换后的Java�?
   * @throws PersistenceException 转换失败时抛出数据源模块异常
   */
  Object convertForJava(Object value, Class<?> targetType) throws PersistenceException;
