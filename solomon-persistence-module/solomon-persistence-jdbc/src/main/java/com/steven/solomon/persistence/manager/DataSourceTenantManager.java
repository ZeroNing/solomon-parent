...
public class DataSourceTenantManager {
   ...
   * 构造数据源租户运行时管理器�?
   *
   * @param properties 数据源配置对象，用于维护运行时租户配置缓�?
   * @param factory 动态数据源工厂，用于根据租户配置创�?Hikari �?Druid 数据�?
   * @param context 数据源租户上下文，用于注册和注销可切换的数据源实�?
   */
   ...
   *
   * @param tenantCode 租户编码，不能为空，后续切换数据源时使用该编�?
   ...
   * @throws PersistenceException 租户编码为空时抛�?
   */
   ...
   * @return 已注册返�?{@code true}，否则返�?{@code false}
   */
