 ...
 * {@link SqlTypeConverter} 做类型转换，解决LocalDateTime、Date、Long等类型在不同数据�?
 * 驱动下返回类型不一致的问题�?/p>
 *
 * @param <T> 查询结果实体类型
 */
public class SqlResultRowMapper<T> implements RowMapper<T> {
   ...
   * 构造结果映射器�?
   *
   * @param resultType 查询结果实体类型，必须存在无参构造方�?
   */
   ...
   *
   * @param resultType 查询结果实体类型，必须存在无参构造方�?
   * @param converterRegistry SQL类型转换器注册器，用于字段赋值前的类型转�?
   */
   ...
   *
   * @param rs JDBC结果集，调用方保证游标已经定位到当前�?
   * @param rowNum 当前行号，从0开�?
   * @return 映射后的实体对象
   * @throws SQLException JDBC读取失败时抛�?
   */
  @Override
