...
public class DataSourceTenantManager {
   ...
   * @throws DataSourceException 租户编码为空、配置为空或数据源创建失败时抛出
   */
  public synchronized DataSource addOrUpdateTenant(
      String tenantCode,
      TenantDataSourceProperties tenantProperties) throws DataSourceException {
   ...
   * @return 新创建并注册成功的数据源实例
   * @throws DataSourceException 租户已存在、配置为空或数据源创建失败时抛出
   */
  public synchronized DataSource addTenant(
    ...
    if (context.isRegistered(normalizedTenantCode)) {
      throw new DataSourceException(DataSourceErrorCode.DATA_SOURCE_TENANT_ALREADY_EXISTS,
   ...
   * @throws DataSourceException 租户编码为空时抛出
   */
  public synchronized DataSource removeTenant(String tenantCode) throws DataSourceException {
    String normalizedTenantCode = requireTenantCode(tenantCode);
