 ...
 */
public class PersistencePageParam implements Serializable {
   ...
   * @return 分页参数对象
   */
  public static PersistencePageParam of(int pageNo, int pageSize) {
    PersistencePageParam param = new PersistencePageParam();
   ...
   * 设置每页条数�?
   *
   * @param pageSize 每页条数；小�?时执行层会按1处理
   */
   ...
   * 设置原始排序表达式�?
   *
   * @param orderBy 排序表达式，不包�?{@code ORDER BY}，例�?{@code "id DESC"}
   */
   ...
   * 链式设置原始排序表达式�?
   *
   * @param orderBy 排序表达式，不包�?{@code ORDER BY}
   * @return 当前分页参数
   */
  public PersistencePageParam orderBy(String orderBy) {
    setOrderBy(orderBy);
    return this;
  }
   ...
   * 追加排序字段�?
   *
   * @param field 排序字段名或表达�?
   * @param method 排序方向；为空时默认倒序
   * @return 当前分页参数
   */
  public PersistencePageParam sort(String field, OrderByEnum method) {
   ...
   * @return 当前分页参数
   */
  public PersistencePageParam asc(String field) {
   ...
   * @return 当前分页参数
   */
  public PersistencePageParam desc(String field) {
   ...
   * 设置游标字段�?
   *
   * @param seekColumn 游标字段名或表达式，建议使用有索引且排序稳定的字�?
   */
   ...
   * 链式设置游标字段�?
   *
   * @param seekColumn 游标字段名或表达�?
   * @return 当前分页参数
   */
  public PersistencePageParam seekColumn(String seekColumn) {
   ...
   * 设置上一页最后一条记录的游标值�?
   *
   * @param lastValue 游标值；为空时框架会在大页码场景下自动查询锚�?
   */
   ...
   * @return 当前分页参数
   */
  public PersistencePageParam lastValue(Object lastValue) {
   ...
   * 设置是否按升序游标分页�?
   *
   * @param asc true表示升序并使用大于游标值，false表示降序并使用小于游标�?
   */
   ...
   * @return 当前分页参数
   */
  public PersistencePageParam seekAsc() {
    this.asc = true;
    return this;
  }
   ...
   * @return 当前分页参数
   */
  public PersistencePageParam seekDesc() {
    this.asc = false;
    return this;
  }
   ...
   * 生成ORDER BY后面的排序表达式�?
   *
   * @return 排序表达式，例如 {@code "id DESC,create_time ASC"}；无排序时返回空字符�?
   */
